...
const OVERSEAS_KEYWORDS = ["QDII", "纳斯达克", "纳指", "NASDAQ", "Nasdaq", "标普", "美国", "全球", "境外", "恒生", "香港"]
const OVERSEAS_THEME_KEYWORDS = OVERSEAS_KEYWORDS.filter((keyword) => keyword.toLowerCase() !== "qdii" && keyword !== "标普")
...
export function getClassificationScope(name: string, type: string): ClassificationScope {
  ...
  if (sources.some((source) => source === "type-qdii" || source === "type-overseas" || source === "name-qdii")) return "strong"
  return "supplemental"
}
...
export function getRegionCategory(name: string, type: string): RegionCategory {
  ...
  if (includesAnyKeyword(text, ["中国互联网", "中概", "中国互联", "大中华", "中国世纪", "中国中小盘", "海外中国", "港美互联"])) return "china-offshore"
  if (includesAnyKeyword(text, ["亚太", "亚洲"])) return "asia-pacific"
  if (isNasdaqLike(name) || includesAnyKeyword(text, ["标普500", "S&P 500", "S&P500", "SP500", "道琼斯", "美国", "美股", "纳斯达克", "纳指"])) return "us"
...
export function getThemeCategory(name: string, type: string): ThemeCategory {
  ...
  if (includesAnyKeyword(text, ["美元债", "债券", "债基", "债", "高收益", "收益债", "信用债", "国债", "政府债", "公司债", "企业债", "投资级", "短债", "中短债", "固收", "票据", "亚洲债", "全球债", "海外债", "新兴市场债"])) return "bond"
  ...
  if (includesAnyKeyword(text, ["港股创新药", "香港创新药", "港股医药", "香港医药", "港股医疗", "香港医疗", "港股生物", "香港生物"])) return "hongkong-biotech"
  if (includesAnyKeyword(text, ["港股互联网", "香港互联网", "海外中国互联网", "中国互联网", "中概互联网", "中概股", "中国互联", "中概互联"])) return "china-internet"
  if (includesAnyKeyword(text, ["生物科技", "生物医药", "医疗保健", "医疗健康", "医药", "医疗", "创新药"])) return "biotech"
  ...
  if (text.includes("越南")) return "vietnam"
  if (includesAnyKeyword(text, ["德国", "DAX"])) return "germany"
  if (includesAnyKeyword(text, ["黄金", "原油", "石油", "油气", "能源", "天然气", "商品", "有色", "资源"])) return "commodity"
